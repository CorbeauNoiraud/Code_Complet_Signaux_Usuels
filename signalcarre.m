...
close all
%*******************************************************************************
%Script d'un signal carré à partir d'un signal harmonique fondamental
%INPUT: A = amplitude
...
M2 = cumsum(RR);
mesh(M2)
title(['Signal en fonction du nombres d''harmoniques et du temps'])
xlabel('temps en (s)')
...
set(ZB,'fontsize',7,'position',[0.5000 -0.83 -1])
ylabel(['y = ',num2str(A),'*sin(2*pi*',num2str(f0),'*t)'])
...
set(ZC,'fontsize',7,'position',[0.5000 -0.83 -1])
ylabel(['y = ',num2str(A),'*sin(2*pi*',num2str(f0),'*t)'])
